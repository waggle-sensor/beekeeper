...
class BeekeeperDB:
        ...
        while True:
            try:
                ...
                if count > retries:
                    raise
                logger.error(
                    f"Could not connnect to database, error={e}, retry in 2 seconds"
                )
                time.sleep(2)
                count += 1
                continue
    ...
    def nodes_log_add(
        self, logData, lock_tables=True, lock_requested_by="", replay=True
    ):
        ...
        values = []
        for log in logData:
            effective_time = dateutil.parser.parse(log["effective_time"]).replace(
                microsecond=0
            )
            ...
            if not field_name in table_fields_index[table_name]:
                raise Exception(
                    f"{field_name} not a valid field name in table {table_name}"
                )
    ...
    def replay_log(self, replay_from_timestamp=None, get_locks=True):
        ...
        if replay_from_timestamp:
            ...
            debug_stmt = stmt
            debug_stmt = debug_stmt.replace(
                "%s", f'"{replay_from_timestamp.isoformat()}"', 1
            )
    ...
    def insert_object(self, table_name, row_object, force=False, commit=True):
        ...
        if (not force) and self.cur.rowcount != 1:
            raise Exception(
                f"insertion went wrong (self.cur.rowcount: {self.cur.rowcount})"
            )
