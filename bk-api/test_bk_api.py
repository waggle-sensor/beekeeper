...
def app():
    ...
    if os.getenv("TESTING") != "1":
        raise RuntimeError(
            "Bailing out on running tests as they wipe the database! If you are not running in production and are sure you want to proceed, you must set the env var TESTING=1."
        )
...
def test_registration_invalid_node_id(client):
    for node_id in [
        ...
        "NOLOwERCASE",
        "AVOIDUSING_",
        "AREALLYREALLYREALLYLONGNODEIDTHATISNOTALLOWED",
    ]:
        r = client.post(f"/register?node_id={node_id}")
        assert r.status_code == HTTPStatus.BAD_REQUEST
...
def test_registration_with_specific_beehive(client):
    ...
    r = client.post(
        "/beehives", data=json.dumps({"id": beehive_id, "key-type": "rsa-sha2-256"})
    )
...
def test_assign_node_to_beehive(client):
    ...
    rv = client.post(
        f"/beehives/{beehive_id}", content_type="multipart/form-data", data=data
    )
    result = rv.get_json()
    assert "modified" in result
    ...
    rv = client.post(
        f"/node/{node_id}",
        data=json.dumps({"assign_beehive": beehive_id, "deploy_wes": True}),
    )
    result = rv.get_json()
    ...
    data.append(
        {
            ...
            "field_value": "project_X",
            "effective_time": (test_time - datetime.timedelta(days=2)).isoformat(),
        }
    )
        ...
        data.append(
            {
                ...
                "field_value": "project_X",
                "effective_time": (
                    test_time - datetime.timedelta(days=2, minutes=i)
                ).isoformat(),
            }
        )
    data.append(
    ...
    data.append(
        {
            ...
            "field_name": "mode",
            "field_value": "failed",
            "effective_time": (test_time - datetime.timedelta(days=3)).isoformat(),
        }
    )
    data.append(
        {
    ...
    data.append(
        {
            ...
            "operation": "insert",
            "field_name": "name",
            "field_value": "Rumpelstilzchen2",
            "effective_time": (test_time - datetime.timedelta(days=4)).isoformat(),
        }
    )
    data.append(
        {
            ...
            "field_value": (test_time - datetime.timedelta(days=5)).isoformat(),
            "effective_time": (test_time - datetime.timedelta(days=5)).isoformat(),
        }
    )
    data.append(
        {
...
def test_credentials(client):
    ...
    rv = client.post(
        ...
        data=json.dumps({"ssh_key_private": "x", "ssh_key_public": "y"}),
    )
    assert rv.status_code == HTTPStatus.OK
...
def test_beehives(client):
    ...
    rv = client.post(
        "/beehives", data=json.dumps({"id": "test-beehive", "key-type": "rsa-sha2-256"})
    )
    result = rv.get_json()
    assert "modified" in result
    ...
    rv = client.post(
        "/beehives/test-beehive", content_type="multipart/form-data", data=data
    )
    result = rv.get_json()
    assert "modified" in result
...
def rand_beehive_id():
    return rand_string(
        3, 64, "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789-_"
    )
...
class MockNodeSubprocessProxy:
    ...
    def __init__(self, node_id, return_values=[]):
        ...
        self.return_values = {
            tuple(cmd): (returncode, output)
            for cmd, returncode, output in return_values
        }
