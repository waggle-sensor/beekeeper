...
NEW_ID=$(docker run --rm  --network beekeeper_default -v `pwd`:/keys/:ro -w /keys -v beekeeper-config_bk-secrets:/usr/lib/sage/:ro sagecontinuum/bk-config bash -c 'apt-get install -y jq ; ssh -o StrictHostKeyChecking=no  sage_registration@bk-sshd -p 22 -i register.pem register 0000000000000001 | jq -r -j .id' | tail -n 1)
